...
namespace Strabo.Core.ImageProcessing
{
    public static class ImageUtils
    {
        ...
        {
            using (var img = new Image<Gray, byte>(srcimg))
            {
        ...
        {
            using (var ip1 = new Image<Gray, byte>(img1))
            using (var ip2 = new Image<Gray, byte>(img2))
            {
        ...
        public static Bitmap AnyToFormat24bppRgb(Bitmap srcimg)
        {
            ...
            {
                var tmp = new Bitmap(srcimg.Width, srcimg.Height, PixelFormat.Format24bppRgb);
        ...
        public static Bitmap AnyToFormat32bppRgb(Bitmap srcimg)
        {
            ...
            {
                var tmp = new Bitmap(srcimg.Width, srcimg.Height,
        ...
        public static int GetNumberOfUniqueColors(Bitmap srcimg)
        {
            ...
            var srcFmt = PixelFormat.Format24bppRgb;
            ...
            var srcOffset = srcData.Stride - width * 3;

            var cnt = new HashSet<int>();
            // do the job
            unsafe
            {
        ...
        public static Bitmap Array1DToBitmapRGB(int[] input, int width, int height)
        {
            ...
            var dstData = dstImg.LockBits(
                new Rectangle(0, 0, width, height),
                ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb);

            var dstOffset = dstData.Stride - width * 3;
        ...
        public static bool[,] BitmapToArray2D(Bitmap srcimg, int threshold, int margin)
        {
            ...
            var image = new bool[height, width];
            // do the job
            unsafe
            {
                var src = (byte*) srcData.Scan0.ToPointer();
        ...
        public static Bitmap Array2DToBitmap(bool[,] input, int margin)
        {
            ...
            var dstImg = CreateBlankGrayScaleBmp(width, height);
            ...
            var dstOffset = dstData.Stride - width;
            ...
            unsafe
            {
                ...
                for (var y = 0; y < height; y++)
                {
                    for (var x = 0; x < width; x++, dst++)
                        if (y >= margin && y < height - margin && x >= margin && x < width - margin)
                            if (input[y, x]) *dst = 0;
                            else if (input[y, x] == false) *dst = 255;
                            else *dst = 128;
                        else *dst = 255;
                    dst += dstOffset;
                }
        ...
        public static Bitmap ArrayBool2DToBitmap(bool[,] input)
        {
            ...
            var dstImg =
                new Bitmap(width, height,
                    PixelFormat.Format8bppIndexed); // AForge.Imaging.Image.CreateGrayscaleImage(width, height);
        ...
        public static Bitmap toGray(Bitmap srcimg)
        {
            if (srcimg.PixelFormat != PixelFormat.Format8bppIndexed)
                return ConvertToGrayScale(srcimg);
            return srcimg;
        }
        ...
        public static Bitmap toRGB(Bitmap srcimg)
        {
            if (srcimg.PixelFormat == PixelFormat.Format8bppIndexed)
            {
                ...
                var dstImg =
                    new Bitmap(width, height,
                        PixelFormat.Format24bppRgb); // AForge.Imaging.Image.CreateGrayscaleImage(width, height);
                ...
                var srcOffset = srcData.Stride - width;
                var dstOffset = dstData.Stride - width * 3;
                // do the job
                unsafe
                {
                    ...
                    for (var y = 0; y < height; y++)
                    {
                        for (var x = 0; x < width; x++, dst += 3, src++)
                        {
                            //*dst = *src;
                            dst[0] = src[0];
                            dst[1] = src[0];
